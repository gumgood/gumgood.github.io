...
함수를 게산 그래프로 나타내는 이유는 **역전파(backpropagation)**를 사용하기 위함이다. 역전파는 계산 그래프 내부의 모든 변수에 대해 chain rule을 재귀적으로 사용해서 gradient를 계산하는 기술이다.
...
이때 $x$, $y$에 대한 $q$의 gradient $(\dfrac{\partial q}{\partial x}, \dfrac{\partial q}{\partial y})$는 $1$이다. 단순한 덧셈에 대해 편미분한 결과다.
...
![](../assets/image/2022-10-05-cs231n-4-1/image3.png)
...
$$
\dfrac{\partial f}{\partial y} = \dfrac{\partial q}{\partial f} \dfrac{\partial y}{\partial q}
$$

즉, $y$에 대한 $f$의 미분 $\dfrac{\partial f}{\partial y}$는 $q$에 대한 $f$의 미분 $\dfrac{\partial q}{\partial f}$과 $y$에 대한 $q$의 미분 $\dfrac{\partial y}{\partial q}$의 곱으로 나타낼 수 있다. 각 항은 $z(=-4)$, $1$과 같으므로 gradient는 $-4$이다.
...
![](../assets/image/2022-10-05-cs231n-4-1/image8.png)

앞에서 설명한 역전파는 계산 그래프 안의 모든 노드에 대해 진행됐다. 그런데 여기서 주목할 점은 각 노드의 계산에는 오직 주변 노드의 정보만 있으면 충분했다.

위 그림은 어떤 노드와 노드의 local 입력 $x$, $y$를 나타낸 것이다. 입력은 노드에 흘러들어와 출력 $z$을 얻게 된다. 이 노드는 위에서 흘러온 gradients $\dfrac{\partial L}{\partial z}$를 통해 **local gradient**를 계산할 수 있다. $x$에 대한 $z$의 gradient $\dfrac{\partial z}{\partial x}$와 $y$에 대한 $z$의 gradient $\dfrac{\partial z}{\partial y}$이다. 각 노드는 덧셈 또는 곱셈과 같은 간단한 형태의 연산이기 때문에 아주 쉽게 gradient를 구할 수 있고 복잡한 미적분학 없이 gradient를 계산할 수 있다.
...
이 함수는 4개의 노드($+$, $*$, $\exp$, $1/x$)로 나타낼 수도 있겠지만, 이를 직접 미분했을 때 나오는 식인 $(1-\sigma(x))(\sigma(x))$를 계산해냈다면 함수 자체를 큰 노드로 만들 수 있다. 즉, local gradient를 명시할 수 있다면 더 복잡한 노드도 만들 수 있다.
...
![](../assets/image/2022-10-05-cs231n-4-1/image10.png)
...
$x$는 변수 $i$에 의해 여러 요소들 $q_i$와 연결되어 있다. 그리고 chain rule에 따라 최종 출력 $f$에 $q_i$와 같은 중간 요소들이 미치는 영향을 계산하고, 이 노드에서 $x$가 각 중간 요소 $q_i$들이 미치는 영향인 local gradient를 계산한 뒤 이들을 합쳐 gradient를 계산하는 것이다.
...
![](../assets/image/2022-10-05-cs231n-4-1/image13.png)
...
만약 입력 $100$개를 묶은 배치로 처리하는 경우 노드는 더 효율적이지만 Jacobian 행렬은 $4,096,000 \times 4,096,000$으로 너무 커진다. 실제로는 이 거대한 행렬을 계산할 필요가 없다. Jacobian 행렬이 어떻게 생겼는지 생각해보면 알 수 있는데 먼저 행렬의 각 원소인 편미분에 대해 생각해보자. 요소별로 $0$과의 $\max$연산을 하는 노드에서 어떤 입력 차원이 어떤 출력 차원에 영향을 줄까? 당연하게도 요소별 연산이므로 각 차원은 같은 차원에 영향을 준다. 이는 행렬에서 대각원소에 위치한 편미분들과 같고 나머지는 영향이 없기 때문에 Jacobian 행렬은 대각행렬이 된다.
...
$n$차원의 $x$와 $n \times n$차원의 $W$에 대한 함수 $f(x,W) = \lVert W \cdot x \rVert ^2$를 에로 들어보자. 계산 그래프는 위에서 볼 수 있듯이 $W$와 $x$를 곱셈 노드로 이은 뒤, $L2$ 노드가 뒤따른다.

![](../assets/image/2022-10-05-cs231n-4-1/image15.png)

$n=2$를 에로 들면, $x$는 $2$차원 벡터가 되고 $W$는 $2 \times 2$ 행렬이 된다. 그리고 중간 노드인 $W$와 $x$의 곱셈 결과를 $q$라고 하자. $q$는 $W$와 $x$의 요소별 곱셈의 합으로 나타낼 수 있다. 그리고 이제 $f$를 $q$에 대해 쓰면 $q$의 L2 norm 값과 같고 $q_1^2$부터 $q_n^2$까지 합친 것과 같다.

![](../assets/image/2022-10-05-cs231n-4-1/image16.png)

이제 역전파를 해보자. $f$에 대한 $f$의 gradient는 당연히 $1$로 시작한다. 이제 L2 노드 뒤로 전파해보자. L2 노드 이전 중간 변수인 $q$에 대한 gradient를 구해 전파해야 한다. 이때 $q$는 2차원 벡터다. 즉, $q$ 각각의 요소가 $f$의 최종 출력에 영향을 얼마나 미쳤는지를 벡터로 구해야 한다.
...
이때 $\mathbf{1}_{k=i}$는 $k=i$이면 $1$, 아니면 $0$이다.
...
![](../assets/image/2022-10-05-cs231n-4-1/image18.png)

마지막으로 x에 대한 gradient를 구하자. $x$의 $i$번째 요소 $x_i$에 대한 $q$의 $k$번째 요소 $q_k$에 대한 미분은 $W_{k,i}$임을 알 수 있다. 마찬가지로 chain rule을 적용해 식을 정리하면 $\nabla _x f = 2W^T \cdot q$과 같은 식을 얻을 수 있다. 그리고 이는 $x$와 같은 모양을 가지는 $x$에 대한 gradient가 된다.
